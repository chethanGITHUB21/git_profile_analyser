...
The migration file intentionally resets the project schema:

- Drops old views.
- Drops old redundant tables.
- Drops and recreates `users` and `repository`.
- Recreates analytics views.

Back up your database before running migration on important data:

```bash
mysqldump -u username -p github_profile_analyser > backup.sql
```

Restore a backup if needed:

```bash
mysql -u username -p github_profile_analyser < backup.sql
```

## GitHub Token Security

Do not commit `.env` or any GitHub token. This repository includes `.gitignore` rules to keep `.env` files out of Git.
...
5. Create a new token with the minimum required permissions.
6. Update `.env`.
7. Restart the server.

For public profile and public repository reads, avoid broad scopes unless you truly need them.
